...
vignette: >
  %\VignetteIndexEntry{Water Quality Analysis Report}
...
signif_sym  <- function(x) {
  if (length(x) > 1) return(lapply(x, signif_sym) |> unlist())
  if (x < 1e-3) {
    "***"
  } else {
    ""
...
* Built environments (urban/developed areas)
...
[1]: https://www.researchgate.net/publication/359937074_Canada%27s_human_footprint_reveals_large_intact_areas_juxtaposed_against_areas_under_immense_anthropogenic_pressure?utm_source=chatgpt.com "(PDF) Canada's human footprint reveals large intact areas ..."
...
```{R, fig.cap="Figure 1 - Comparison between the cumuative threat values of the Hirsh-Pearson and the Theobald datasets."}
...
Overall, results obtained with the two cumulative threat datasets were highly consistent across temporal (daily, monthly, yearly) and spatial scales (HydroBASINS levels 7 and 12) (Figure 2).

```{R, fig.cap="Figure 2 - Effect size for the main regression models where. The role of cumulative threats (Hirsh-Pearson and Theobald) datasets in explaining the porbbaiity o execeeding gudeline is explored at two spatial sclaes (hydrobasins level 7 and level 12) for three different temporal scale: day, month and year."}
knitr::include_graphics("figs/fig2.png")
```

The explanatory power of cumulative threats in predicting guideline exceedance varied substantially among water-chemistry variables. The strongest effect sizes were observed for total dissolved solids (TDS), where effects were strongly negative (up to −2.77), and for dissolved chloride, where effects were strongly positive (up to 1.97), see Table. Moderate but significant positive effects were observed for turbidity and dissolved oxygen, whereas smaller negative effects were detected for pH and nitrate. Effects were very small or non-significant for conductivity and total phosphorus.

Except for TDS (up to 15.08) and dissolved chloride (up to 40.93%), the percentage of deviance explained by the models was generally low.


```{R, fig.cap="Figure 3 - Example of regression plots using the Theobald dataset and hydrobasings level 12"}
knitr::include_graphics("figs/fig3.png")
```


Results for the individual Hirsh-Pearson components were more difficult to interpret (Figure 4). Although some strong effect sizes were detected, overall model fit was low and several estimates appeared influenced by high-leverage observations. In addition, due to strong imbalance in the distribution of binary outcomes (0 vs 1), three of the twelve components had to be excluded from the analysis.



```{R, fig.cap="Figure 4 - Same Figure as Figure 2 for all components of the Hirsh-Pearson datasets. Only the year scale is investigated."}
...
```{R table_res_main, tab.cap = "Table 1 = Results for the main set of regressions."}
path_results("res_main.csv") |>
  read.csv() |>
  dplyr::select(stressor, explanatory_var, response_var, effect, pval, expl_dev) |>
  dplyr::mutate(
    expl_dev = signif(100 * expl_dev, 4),
    Effect = paste0(signif(effect, 4), signif_sym(pval)),
    Model = paste0(response_var, " ~ ", explanatory_var)
  ) |>
