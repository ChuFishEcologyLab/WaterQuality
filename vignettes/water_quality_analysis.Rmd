...
```{R, fig.cap="Comparison between the cumuative threat values of the Hirsh-Pearson and the Theobald datasets."}
...
Overall, results obtained with the two cumulative datasets were highly consistent across temporal (daily, monthly, yearly) and spatial scales (HydroBASINS levels 7 and 12) (Figure 2).

```{R, fig.cap="Effect size for the main regression models where. The role of cumulative threats (Hirsh-Pearson and Theobald) datasets in explaining the porbbaiity o execeeding gudeline is explored at two spatial sclaes (hydrobasins level 7 and level 12) for three different temporal scale: day, month and year."}
knitr::include_graphics("figs/fig2.png")
```

The explanatory power of cumulative threats in predicting guideline exceedance varied substantially among water-chemistry variables. The strongest effect sizes were observed for total dissolved solids (TDS), where effects were strongly negative (up to −2.77), and for dissolved chloride (1.968), where effects were strongly positive. Moderate but significant positive effects were observed for turbidity and dissolved oxygen, whereas smaller negative effects were detected for pH and nitrate. Effects were very small or non-significant for conductivity and total phosphorus.
...
```{R, fig.cap="Example of regression plots using the Theobald dataset and hydrobasings level 12"}
knitr::include_graphics("figs/fig3.png")
```


Results for the individual Hirsh-Pearson components were more difficult to interpret. Although some strong effect sizes were detected, overall model fit was low and several estimates appeared influenced by high-leverage observations. In addition, due to strong imbalance in the distribution of binary outcomes (0 vs 1), three of the twelve components had to be excluded from the analysis.



```{R, fig.cap="Same Figure as Figure 2 for all components of the Hirsh-Pearson datasets. Only the year scale is investigated."}
...
```{R table_res_main, tab.cap = "Results for the main set of regressions."}
